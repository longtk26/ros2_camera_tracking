    ...
    def update_graph(self):
        ...
        try:
            if self.START_STANDLEY_ALGORITHM:
                ...
                self.get_logger().info(f"Published graph data: {self.closest_point[0]}:{self.x_current}:{self.closest_point[1]}:{self.y_current}:{self.min_distance}:{self.average_min_distance}")
        except Exception as e:
